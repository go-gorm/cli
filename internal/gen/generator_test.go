...
func TestGeneratorWithQueryInterface(t *testing.T) {
	inputPath, err := filepath.Abs("../../examples/gen/query.go")
	if err != nil {
		t.Fatalf("failed to get absolute path: %v", err)
	}

	goldenPath, err := filepath.Abs("../../examples/gen/output/query.go")
	if err != nil {
...
func TestProcessStructType(t *testing.T) {
	...
	file, err := parser.ParseFile(fileset, "../../examples/gen/models/user.go", nil, parser.AllErrors)
	if err != nil {
